...
public class Lwjgl3GlVao implements GlVao {
	...
	@Override
	public void attribPointer(int bindingPoint, Datatypes.PrimitiveDataType<?> dataType, boolean normalized, int stride,
			int offset) {
		log.trace("attribPointer({}, {}, {}, {}, {})", bindingPoint, dataType, normalized, stride, offset);
		GL30.glVertexAttribPointer(bindingPoint, dataType.components(), convertDataType(dataType), normalized, stride,
				offset);
	}

	@Override
	public void enableAttributes(VertexDataPoints.VertexDataPoint<?> vertexDataPoints) {
		log.trace("enableAttributes({})", vertexDataPoints);
		GL30.glEnableVertexAttribArray(vertexDataPoints.bindingPoint());
	}
