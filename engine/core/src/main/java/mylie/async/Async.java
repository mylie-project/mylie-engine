...
public class Async {
	...
	public static <R, O> Result<R> async(ExecutionMode executionMode, long version, Functions.F0<R, O> function,
			O object) {
	...
	public static <R, O, P0> Result<R> async(ExecutionMode executionMode, long version, Functions.F1<R, O, P0> function,
			O object, P0 param0) {
		int hashCode = hash(function, object);
		Result<R> result = executionMode.cache().get(hashCode, version);
	...
	public static <R, O, P0, P1> Result<R> async(ExecutionMode executionMode, long version,
	...
	public static <T, O> Iterable<Result<T>> async(List<?> objects, Class<? extends O> type,
			java.util.function.Function<O, Result<T>> function) {
	...
	public static <T, O, P0> Iterable<Result<T>> async(List<?> objects, Class<? extends O> type,
			BiFunction<O, P0, Result<T>> function, P0 param) {
		List<Result<T>> results = new ArrayList<>();
		for (Object object : objects) {
			if (type.isInstance(object)) {
