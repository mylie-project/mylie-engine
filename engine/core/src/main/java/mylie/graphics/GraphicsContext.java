...
public abstract class GraphicsContext
		implements
			Configurable<GraphicsContext, GraphicsContext.Option<?>>,
			PropertiesAA<GraphicsContext, GraphicsContext.Properties<?>> {
	private static int contextCount = -1;

	@Getter(AccessLevel.PUBLIC)
	private final mylie.util.properties.Properties<GraphicsContext, GraphicsContext.Properties<?>> properties = new mylie.util.properties.Properties.Map<>(
			AutoIncremented::new);
	...
	/**
	 * Destroys the current graphics context, releasing any associated resources and
	 * performing necessary cleanup operations. This abstract method should be
	 * implemented by subclasses to define specific destruction logic for a
	 * particular graphics context implementation.
	 *
	 * @return a Result<Boolean> indicating the success or failure of the destroy
	 *         operation.
	 */
	protected abstract Result<Async.Void> destroy();

	/**
	 * Swaps the rendering buffers to display the current drawing frame. This
	 * operation typically involves flipping the back buffer to become the visible
	 * front buffer and vice versa. This method is intended to be implemented by
	 * subclasses to provide specific logic for buffer swapping based on the
	 * rendering context being used.
	 *
	 * @return a Result<Boolean> indicating the success or failure of the buffer
	 *         swap operation.
	 */
	...
	public interface VideoMode {
		 ...
		 * its particular video mode settings, which dictate how the graphical content
		 * is rendered when in full-screen mode.
		 *
		 * @param display
		 *            The display on which the full-screen mode is to be set.
		 * @param videoMode
		 *            The specific video mode configuration to use for full-screen
		 *            rendering.
		 */
		 ...
		 * application, offering flexibility in terms of window size and position on the
		 * screen, while implementing the VideoMode interface.
		 *
		 * @param display
		 *            The display on which the windowed mode is to be set.
		 * @param size
		 *            The dimensions of the window, specified by a Vector2ic object.
		 * @param position
		 *            The position of the window on the screen, specified by a Vector2ic
		 *            object. The static field Centered provides a default positioning
		...
		/**
		 * The WindowedFullscreen record represents a hybrid video mode within a
		 * graphical application. It combines aspects of both windowed and fullscreen
		 * modes, allowing content to be displayed in what is effectively fullscreen,
		 * but within windowed constraints. This mode is useful for applications that
		 * require the immersion of fullscreen while retaining some aspects of windowed
		 * operation, such as window controls or task switching.
		 *
		 * @param display
		 *            The display on which the windowed fullscreen mode is to be set.
		 */
