...
public class GlBufferManager implements ApiManager {
    ...
    public void bindBuffer(RenderTask renderTask,Datatypes.DataBuffer<?> buffer, Target target, AccessMode accessMode){
        renderTask.subTask(() -> bindBuffer(buffer,target,accessMode));
    }

    protected boolean bindBuffer(Datatypes.DataBuffer<?> buffer, Target target, AccessMode accessMode){
        boolean newBuffer=false;
        BufferHandle handle = bufferHandles.get(buffer);
        if(handle==null){
            handle=new BufferHandle();
            bufferHandles.put(buffer,handle);
            glBuffer.createBuffer(handle);
            newBuffer=true;
        }
        glBuffer.bindBuffer(handle,target);
        if(handle.version()<buffer.version()){
            if(handle.size==-1){
                glBuffer.bufferData(buffer, target, accessMode);
                handle.size=buffer.size();
                handle.version(buffer.version());
            }else{
                if(handle.size==buffer.size()){
                    glBuffer.updateBufferData(buffer,target,accessMode);
                }else{
                    if(handle.size<buffer.size()){
                        glBuffer.bufferData(buffer, target, accessMode);
                        handle.size=buffer.size();
    ...
    protected boolean checkBindBuffer(Datatypes.DataBuffer<?> buffer,Target target,AccessMode accessMode){
        if(!checkBuffer(buffer)) {
            return bindBuffer(buffer, target, accessMode);
        }
        return false;
    }
